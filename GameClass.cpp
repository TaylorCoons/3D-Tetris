...
bool GameClass::InitD3D() {
	...
	HRESULT adaptersLeft = S_OK;
	int adapterIndex = 0;
	do {
		adaptersLeft = dxgiFactory->EnumAdapters1(adapterIndex, &pAdapter);
	...
	} while (adaptersLeft != S_OK);

	//for (int adapterIndex = 0; dxgiFactory->EnumAdapters1(adapterIndex, &pAdapter) != S_OK; adapterIndex++) {
	//	// Get adapters description
	//	DXGI_ADAPTER_DESC1 adapterDesc;
	//	pAdapter->GetDesc1(&adapterDesc);
	//	if (adapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) {
	...
	backBufferDesc.Width = rc.right - rc.left; // buffer width
	backBufferDesc.Height = rc.bottom - rc.top; // buffer height
	backBufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; // format of the buffer (rgba 32 bits, 8 bits for each chanel)

	
	IDXGISwapChain* pTempSwapChain = nullptr;
	hr = dxgiFactory->CreateSwapChain(pCommandQueue, &scd, &pTempSwapChain);
	if (FAILED(hr)) {
...
bool GameClass::UpdatePipeline() {
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(pRTVDescHeap->GetCPUDescriptorHandleForHeapStart(), frameIndex, rtvDescSize);
...
bool GameClass::WaitForPreviousFrame() {
	...
	if (fences[frameIndex]->GetCompletedValue() < fenceValue[frameIndex]) {
		...
		// Blocking call pauses CPU until fenceEvent arrives
